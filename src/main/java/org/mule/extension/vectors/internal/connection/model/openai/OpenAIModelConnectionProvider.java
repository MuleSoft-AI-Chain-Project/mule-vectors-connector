...
@Alias("openAI")
@DisplayName("OpenAI")
public class OpenAIModelConnectionProvider extends BaseModelConnectionProvider implements Startable, Stoppable {
  ...
  public ConnectionValidationResult validate(BaseModelConnection connection) {
    try {
      if (connection.isValid()) {
        ...
        return ConnectionValidationResult.failure("Failed to validate connection to OpenAI", null);
      }
    } catch (Exception e) {
      return ConnectionValidationResult.failure("Failed to validate connection to OpenAI", e);
    }
  }
