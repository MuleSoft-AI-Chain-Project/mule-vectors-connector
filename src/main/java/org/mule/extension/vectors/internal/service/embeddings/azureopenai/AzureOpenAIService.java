...
public class AzureOpenAIService implements EmbeddingService {
  ...
  public AzureOpenAIService(AzureOpenAIModelConnection azureOpenAIModelConnection,
  ...
  private CompletableFuture<String> generateTextEmbeddingsAsync(List<String> inputs, String deploymentName) {
    ...
    try {
      ...
      return HttpRequestHelper.executePostRequest(this.azureOpenAIModelConnection.getHttpClient(), url, buildHeaders(), body,
                                                  (int) this.azureOpenAIModelConnection.getTimeout())
  ...
  private String buildUrlForDeployment(String deploymentName) {
    try {
      ...
      String encodedApiVersion =
          URLEncoder.encode(this.azureOpenAIModelConnection.getApiVersion(), StandardCharsets.UTF_8.name());
      return String.format("%s/openai/deployments/%s/embeddings?api-version=%s", this.azureOpenAIModelConnection.getEndpoint(),
                           encodedDeployment, encodedApiVersion);
