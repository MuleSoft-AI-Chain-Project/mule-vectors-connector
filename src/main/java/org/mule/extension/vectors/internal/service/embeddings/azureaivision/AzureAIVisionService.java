...
public class AzureAIVisionService implements EmbeddingService {
  ...
  public AzureAIVisionService(AzureAIVisionModelConnection azureAIVisionModelConnection,
  ...
  private CompletableFuture<String> generateTextEmbeddingsAsync(String text, String modelName) {
    String url = buildUrlWithParams(this.azureAIVisionModelConnection.getEndpoint() + "/computervision/retrieval:vectorizeText",
                                    Map.of("api-version", this.azureAIVisionModelConnection.getApiVersion(), "model-version",
                                           modelName));
    byte[] payload = new JSONObject().put("text", text).toString().getBytes(StandardCharsets.UTF_8);

    return HttpRequestHelper
        .executePostRequest(this.azureAIVisionModelConnection.getHttpClient(), url, buildHeaders("application/json"), payload,
                            (int) this.azureAIVisionModelConnection.getTimeout())
  ...
  private CompletableFuture<String> generateImageEmbeddingsAsync(byte[] imageBytes, String modelName) {
    ...
    return HttpRequestHelper.executePostRequest(this.azureAIVisionModelConnection.getHttpClient(), url, headers, imageBytes,
                                                (int) this.azureAIVisionModelConnection.getTimeout())
        .thenApply(this::handleEmbeddingResponse);
  }
  ...
  public Response<List<Embedding>> embedTexts(List<String> texts) {
    ...
    try {
      for (int index = 0; index < texts.size(); index += 1) {
        String response =
            (String) generateTextEmbeddings(List.of(texts.get(index)), embeddingModelParameters.getEmbeddingModelName());
