...
public class OllamaService implements EmbeddingService {
  ...
  private CompletableFuture<String> generateTextEmbeddingsAsync(String input, String modelName) {
    try {
      ...
      return HttpRequestHelper.executePostRequest(this.ollamaModelConnection.getHttpClient(),
                                                  this.ollamaModelConnection.getBaseUrl() + EMBEDDINGS_ENDPOINT,
                                                  buildJsonHeaders(), body, (int) this.ollamaModelConnection.getTimeout())
  ...
  public Response<List<Embedding>> embedTexts(List<String> texts) {
    {
      ...
      for (int i = 0; i < texts.size(); i++) {
        try {
          String response = (String) generateTextEmbeddings(Collections.singletonList(texts.get(i)),
