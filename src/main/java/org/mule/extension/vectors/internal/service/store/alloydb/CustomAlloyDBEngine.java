    ...
    try {
      try (
        ...
        if (embeddingStoreConfig.getOverwriteExisting()) {
          statement.executeUpdate(String.format("DROP TABLE IF EXISTS \"%s\".\"%s\"", embeddingStoreConfig.getSchemaName(),
        ...
        if (embeddingStoreConfig.getMetadataColumns() != null && !embeddingStoreConfig.getMetadataColumns().isEmpty()) {
          metadataClause = metadataClause + String.format(", %s", embeddingStoreConfig.getMetadataColumns().stream().map(
                                                                                                                         MetadataColumn::generateColumnString)
              .collect(Collectors.joining(", ")));
        }

        if (embeddingStoreConfig.getStoreMetadata()) {
          metadataClause = metadataClause + String
              .format(", %s",
                      (new MetadataColumn(embeddingStoreConfig.getMetadataJsonColumn(), "JSON", true)).generateColumnString());
        }

        String createStatement = embeddingStoreConfig.getOverwriteExisting() ? "CREATE TABLE" : "CREATE TABLE IF NOT EXISTS";
        String query =
            String.format("%s \"%s\".\"%s\" (\"%s\" UUID PRIMARY KEY, \"%s\" TEXT NULL, \"%s\" vector(%d) NOT NULL%s)",
                          createStatement, embeddingStoreConfig.getSchemaName(), embeddingStoreConfig.getTableName(),
