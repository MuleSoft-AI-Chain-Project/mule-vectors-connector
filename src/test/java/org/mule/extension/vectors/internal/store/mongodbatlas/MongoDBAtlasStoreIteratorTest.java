...
class MongoDBAtlasStoreIteratorTest {
    @Mock MongoDBAtlasStoreConnection connection;
    ...
    void constructor_loadsFirstBatch() {
        ...
        when(findIterable.into(any(List.class)))
            .thenAnswer(inv -> { List<Document> l = inv.getArgument(0); l.addAll(docs); return l; })
            .thenAnswer(inv -> { List<Document> l = inv.getArgument(0); return l; }); // empty batch after first
    ...
    void paging_loadsMultipleBatches() {
        ...
        when(findIterable.into(any(List.class))).thenAnswer(inv -> { List<Document> l = inv.getArgument(0); l.addAll(batch1); return l; })
            .thenAnswer(inv -> { List<Document> l = inv.getArgument(0); l.addAll(batch2); return l; })
            .thenAnswer(inv -> { List<Document> l = inv.getArgument(0); return l; }); // empty batch
        MongoDBAtlasStoreIterator<TextSegment> iterator = new MongoDBAtlasStoreIterator<>(connection, "store", queryParameters);
        assertThat(iterator.hasNext()).isTrue();
        assertThat(iterator.next().getId()).isEqualTo("id1");
        assertThat(iterator.hasNext()).isTrue();
    ...
    void next_extractsVectorAndMetadata() {
        ...
        when(findIterable.into(any(List.class))).thenAnswer(inv -> { List<Document> l = inv.getArgument(0); l.add(doc); return l; });
        MongoDBAtlasStoreIterator<TextSegment> iterator = new MongoDBAtlasStoreIterator<>(connection, "store", queryParameters);
        assertThat(iterator.hasNext()).isTrue();
