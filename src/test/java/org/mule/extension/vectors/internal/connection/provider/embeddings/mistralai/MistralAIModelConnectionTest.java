...
class MistralAIModelConnectionTest {

    MistralAIModelConnection conn;
    HttpClient httpClient;

    @BeforeEach
    void setUp() {
        httpClient = mock(HttpClient.class);
        conn = new MistralAIModelConnection("sk-mistral", 12345L, httpClient);
    }
    ...
    void testGetters() {
        ...
        assertEquals(Constants.EMBEDDING_MODEL_SERVICE_MISTRAL_AI, conn.getEmbeddingModelService());
    }
    ...
    void validate401ThrowsModuleException() throws Exception {
        ...
        try (MockedStatic<HttpRequestHelper> mocked = mockStatic(HttpRequestHelper.class)) {
            ...
            assertEquals("Failed to validate connection to Mistral AI", ex.getMessage());
        }
    }
